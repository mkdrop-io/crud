...
module.exports = function (deployer, network, accounts) {
  deployer.then(async () => {
    try {
      ...
      console.log(`Super admin role   [${SUPER_ADMIN_ADDRESS}]: ${await pool.methods.hasRole(SUPER_ADMIN_ROLE, SUPER_ADMIN_ADDRESS).call()}`);
      await pool.methods.addAdmin(SUPER_ADMIN_ROLE, SUPER_ADMIN_ADDRESS).send({
        from: accounts[0]
      });
      console.log(`Super admin role   [${SUPER_ADMIN_ADDRESS}]: ${await pool.methods.hasRole(SUPER_ADMIN_ROLE, SUPER_ADMIN_ADDRESS).call()}`);
      console.log('--------------------------------------------------------------------');

      console.log(`Deployer       [${accounts[0]}]: ${await pool.methods.hasRole(SUPER_ADMIN_ROLE, accounts[0]).call()}`);
      await pool.methods.addAdmin(SUPER_ADMIN_ROLE, accounts[0]).send({
        from: accounts[0]
      });
      console.log(`Deployer       [${accounts[0]}]: ${await pool.methods.hasRole(SUPER_ADMIN_ROLE, accounts[0]).call()}`);
      ...
      console.log(`General partner    [${GENERAL_PARTNER_ADDRESS}]: ${await pool.methods.hasRole(GENERAL_PARTNER_ROLE, GENERAL_PARTNER_ADDRESS).call()}`);
      await pool.methods.grantRole(GENERAL_PARTNER_ROLE, GENERAL_PARTNER_ADDRESS).send({
        from: accounts[0]
      });
      console.log(`General partner    [${GENERAL_PARTNER_ADDRESS}]: ${await pool.methods.hasRole(GENERAL_PARTNER_ROLE, GENERAL_PARTNER_ADDRESS).call()}`);
      console.log('--------------------------------------------------------------------');

      console.log(`Limited partner    [${LIMITED_PARTNER_ADDRESS}]: ${await pool.methods.hasRole(LIMITED_PARTNER_ROLE, LIMITED_PARTNER_ADDRESS).call()}`);
      await pool.methods.grantRole(LIMITED_PARTNER_ROLE, LIMITED_PARTNER_ADDRESS).send({
        from: accounts[0]
      });
      console.log(`Limited partner    [${LIMITED_PARTNER_ADDRESS}]: ${await pool.methods.hasRole(LIMITED_PARTNER_ROLE, LIMITED_PARTNER_ADDRESS).call()}`);
      console.log('--------------------------------------------------------------------');

      console.log(`Team address       [${TEAM_ADDRESS}]: ${await pool.methods.hasRole(TEAM_ROLE, TEAM_ADDRESS).call()}`);
      await pool.methods.grantRole(TEAM_ROLE, TEAM_ADDRESS).send({
        from: accounts[0]
      });
      console.log(`Team address       [${TEAM_ADDRESS}]: ${await pool.methods.hasRole(TEAM_ROLE, TEAM_ADDRESS).call()}`);
      console.log('--------------------------------------------------------------------');

      console.log(`Sturtap team       [${STURTAP_TEAM_ADDRESS}]: ${await pool.methods.hasRole(STURTAP_TEAM_ROLE, STURTAP_TEAM_ADDRESS).call()}`);
      await pool.methods.grantRole(STURTAP_TEAM_ROLE, STURTAP_TEAM_ADDRESS).send({
        from: accounts[0]
      });
      console.log(`Sturtap team       [${STURTAP_TEAM_ADDRESS}]: ${await pool.methods.hasRole(STURTAP_TEAM_ROLE, STURTAP_TEAM_ADDRESS).call()}`);
      console.log('--------------------------------------------------------------------');

      console.log(`Referer            [${REFERER_ADDRESS}]: ${await pool.methods.hasRole(REFERER_ROLE, REFERER_ADDRESS).call()}`);
      await pool.methods.grantRole(REFERER_ROLE, REFERER_ADDRESS).send({
        from: accounts[0]
      });
      console.log(`Referer            [${REFERER_ADDRESS}]: ${await pool.methods.hasRole(REFERER_ROLE, REFERER_ADDRESS).call()}`);
