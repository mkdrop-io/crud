...
//  using SafeERC20 for IERC20;
  ...
  struct Deposit {  
    ...
    function poolDeposit(address _from,address _tokenAddress, uint256 amount, uint8 period, bool requestDeposit) payable external {
        ...
        if(!requestDeposit) {
          doDeposit(msg.sender, lockerIndex);
    ...
    function doDeposit(address client, uint256 index) payable public {
        ...
        Deposit storage deposit = deposits[index];
         ...
         if (deposit._tokenAddress == wETH && msg.value > 0) {
            ...
            IERC20(deposit._tokenAddress).transferFrom(client, locker, deposit.amount);
...
//    setRole_(RR_REFERRER, _referrer);
  ...
  */
  ...
  function getStakeholderBalanceOf(uint8 _for) external pure returns(uint) {
 ...
 function getCollectedAmount_() internal view returns (uint) {
  ...
  function getStakeholderBalanceOf_(uint8 _for) internal pure returns (uint) {
   return _for;
  }
